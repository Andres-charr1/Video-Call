...
export class ChatsPage implements OnInit {
  ...
  async ngOnInit() {
    this.telefono = this.ruta.snapshot.paramMap.get('telefono')!;
    this.idUsuarioActual = localStorage.getItem('user_id') || '';
    this.usuarioActual = await this.usuarioSrv.buscar(this.idUsuarioActual);
    this.otroUsuario = await this.usuarioSrv.buscarPorTelefono(this.telefono);
    const otroTelefono = this.telefono || '';
  ...
  enviarFoto() {
    this.camaraSrv
      .seleccionarImagen()
      .then((blob) => {
        const archivo = new File([blob], `${Date.now()}.jpg`, {
          type: blob.type,
        });
  ...
  async alternarGrabacion() {
    try {
      if (!this.grabando) {
        ...
        const { datosGrabacionBase64, tipoMime } =
          await this.vozSrv.detenerGrabacion();
        this.grabando = false;
